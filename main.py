import discord from discord.ext import commands, tasks from discord import app_commands import os, json, random, string, asyncio, time import gspread from oauth2client.service_account import ServiceAccountCredentials from keep_alive import keep_alive

TOKEN = os.environ["TOKEN"] ACCOUNT_NOTI_CHANNEL = int(os.environ.get("ACCOUNT_NOTI_CHANNEL", 0)) NOTIFY_CHANNEL_ID = int(os.environ.get("NOTIFY_CHANNEL_ID", 0)) SHEET_NAME = "RobloxAccounts"

scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"] creds = ServiceAccountCredentials.from_json_keyfile_dict(json.loads(os.environ["GOOGLE_CREDENTIALS_JSON"]), scope) client = gspread.authorize(creds) sheet = client.open(SHEET_NAME).sheet1

def read_accounts(): accs = {} for row in sheet.get_all_records(): a = str(row.get("Account", "")).strip() if not a: continue accs[a] = { "note": str(row.get("Note", "")).strip(), "otp": str(row.get("otp", "")).strip(), "email": str(row.get("email", "")).strip() } return accs

def save_account(a, note="", otp="", email=""): sheet.append_row([a, note, otp, email, ""])

def delete_account(a): cell = sheet.find(a) if cell and cell.col == 1: sheet.delete_rows(cell.row)

def update_account_field(a, field, val): col_map = {"note": 2, "otp": 3, "email": 4} cell = sheet.find(a) if cell and cell.col == 1 and field in col_map: sheet.update_cell(cell.row, col_map[field], val) return True return False

def generate_name(n=12): s = random.choice(string.ascii_uppercase) + random.choice(string.ascii_lowercase) + random.choice(string.digits) s += "".join(random.choices(string.ascii_letters + string.digits, k=n - 3)) return "".join(random.sample(s, len(s)))

async def send_log(bot, interaction, action): if NOTIFY_CHANNEL_ID: ch = bot.get_channel(NOTIFY_CHANNEL_ID) if ch: await ch.send(f"📝 {interaction.user} dùng lệnh /{interaction.command.name}\n📘 {action}")

class MyBot(commands.Bot): def init(self): intents = discord.Intents.default() intents.message_content = True super().init(command_prefix="!", intents=intents) self.accounts = {}
...
@bot.event async def on_ready(): print(f"🤖 Bot sẵn sàng: {bot.user} (ID: {bot.user.id})")

@bot.event async def on_message(message): if message.author == bot.user or message.content.startswith("/"): return acc = message.content.strip().split()[0] data = bot.accounts.get(acc) if data: try: expire = int(time.time()) + 45 reply = await message.reply( f"📄 Account: {acc}\n" f"📝 Note: {data.get('note','')}\n" f"🔑 OTP: {data.get('otp','')}\n" f"📧 Email: {data.get('email','')}\n" f"⏳ Sẽ bị xoá <t:{expire}:R>" ) await asyncio.sleep(45) await reply.delete() await message.delete() except: pass await bot.process_commands(message)

if name == "main": keep_alive() bot.run(TOKEN)
